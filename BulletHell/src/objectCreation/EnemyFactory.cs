...
namespace BulletHell.ObjectCreation {
    public class EnemyFactory
    {
        ...
        private Enemy makeEnemy(string textureName, int health, Path path, string gunType, double scale)
        {
            ...
            SpiralLocationEquation s = new SpiralLocationEquation(Math.PI, 15, 5);
            BossGun g = new BossGun(path.InitialLocation, -2, new SingleBulletFactory(new LinearLocationEquation(Math.PI/2, .5F)), (float)(Math.PI/2), s, texture2, TEAM.ENEMY);
