...
namespace BulletHell.bullet
{
    public class ShotgunBulletFactory : BulletFactory
    {
        ...
        public override List<Bullet> makeBullets(Vector2 location, Texture2D bulletTexture, TEAM team, double angleOffset)
        {
            ...
            Path pathLeftDiag = new Path(left, location, spread + angleOffset);
            Path pathRightDiag = new Path(right, location, -spread + angleOffset);
            Path pathDown = new Path(center, location, angleOffset);
